...
$(document).on('click', '#preselectedPictures', function(e) {
...
var returned_pictures = null;
var backendCallCompleted = false;

function backendPicturesCall(){
	
	$.ajax({
		type: "GET",
		url: "/getPreselectedPics",
		success: function(data) {
			returned_pictures = JSON.parse(data);
			backendCallCompleted = true;
			
			if(returned_pictures.length != 0) {
				showPredefinedPictures();
			}
...
function showPredefinedPictures(){
	...
	for (; i < returned_pictures.length; i++){
		divItem = document.createElement('div');
		divs += '<div class="item">';
		divs += '<div class="item-content">';
		
		divs += '<img class="img-fluid img-pick preselected" src="'+returned_pictures[i].url+'" id="'+returned_pictures[i].picID+'" onload="refreshGrid()" data-canvas="'+returned_pictures[i].url+'">';
...
function drawCanvasPict(){
	...
	img.onload = function(){
		...
		switch(layout){
			case 0:
				ctx.fillRect(moveXAmount, moveYAmount, 450, 450);
				break;
			case 1:
				ctx.fillRect(moveXAmount, moveYAmount, 150, 450);
				break;
			case 2:
				ctx.fillRect(moveXAmount, moveYAmount, 450, 150);
				break;
			case 3:
				ctx.fillRect(moveXAmount, moveYAmount, 225, 225);
				break;
		}
		...
		switch(layout) {
			case 0:
				...
				while(450 * captureSizeAvailable > canvas.width && captureSizeAvailable > 1) {
				...
				while(450 * captureSizeAvailable > canvas.height && captureSizeAvailable > 1) {
					if(captureSizeAvailable != 1.5) {
						$('#multRadio' + captureSizeAvailable + 'x').attr('disabled', true);
					} else {
						$('#multRadio15x').attr('disabled', true);
			...
			case 1:
				var captureSizeAvailable = 2;
				while(150 * captureSizeAvailable > canvas.width && captureSizeAvailable > 1) {
					if(captureSizeAvailable != 1.5) {
						$('#multRadio' + captureSizeAvailable + 'x').attr('disabled', true);
					} else {
						$('#multRadio15x').attr('disabled', true);
			...
			case 2:
				...
				captureSizeAvailable = 2;
				while(150 * captureSizeAvailable > canvas.height && captureSizeAvailable > 1) {
					if(captureSizeAvailable != 1.5) {
						$('#multRadio' + captureSizeAvailable + 'x').attr('disabled', true);
					} else {
						$('#multRadio15x').attr('disabled', true);
			...
			case 3:
				var captureSizeAvailable = 2;
				while(225 * captureSizeAvailable > canvas.width && captureSizeAvailable > 1) {
					if(captureSizeAvailable != 1.5) {
						$('#multRadio' + captureSizeAvailable + 'x').attr('disabled', true);
					} else {
						$('#multRadio15x').attr('disabled', true);
				...
				captureSizeAvailable = 2;
				while(225 * captureSizeAvailable > canvas.height && captureSizeAvailable > 1) {
					if(captureSizeAvailable != 1.5) {
						$('#multRadio' + captureSizeAvailable + 'x').attr('disabled', true);
					} else {
						$('#multRadio15x').attr('disabled', true);
...
$(document).on("mousemove touchmove", "#mask-canvas", function(event) {
	...
	if(event.type === "touchmove") {
		...
		moveXAmount = (event.offsetX) * (canvas.width / $('#mask-canvas').width());
        moveYAmount = (event.offsetY) * (canvas.height / $('#mask-canvas').height());
...
/*
 * Returns the images selected by the User while creating the Vision.
 */
