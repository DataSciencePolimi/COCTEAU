...
public interface NarrativeRepository extends JpaRepository<Narrative, Integer>{
	 ...
	 */
	@Query(value = "SELECT * FROM narrative WHERE lang = :lang ORDER BY id_narrative", nativeQuery = true)
	...
	@Query(value = "SELECT n.* FROM narrative as n JOIN scenario as s ON n.id_narrative = s.id_narrative WHERE id_scenario = :id_scenario", nativeQuery = true)
	Narrative findByIdScenario(@Param("id_scenario") int id_scenario);
}
