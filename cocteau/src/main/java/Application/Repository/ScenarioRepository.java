...
public interface ScenarioRepository extends JpaRepository<Scenario, Integer>{
	...
	@Query(value = "SELECT s.* FROM scenario as s JOIN referral_code_scenario as rcs on s.id_scenario = rcs.scenario WHERE s.lang = :lang AND rcs.released = 1 AND s.deleted = 0 AND rcs.referral_code IN (SELECT referral_code FROM referral_code as rc JOIN user as u ON rc.id_referral_code = u.referral_code WHERE cookie = :id_user AND rc.value = :refCode)", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM scenario WHERE scenario_group = :scenarioGroup AND lang = :lang", nativeQuery = true)
	Scenario findByScenarioGroupAndLanguage(@Param("scenarioGroup") int scenarioGroup, @Param("lang") String lang);
	...
	@Query(value = "SELECT s.* FROM scenario as s JOIN referral_code_scenario as rcs on s.id_scenario = rcs.scenario WHERE s.lang = :lang AND rcs.released = 1 AND s.deleted = 0 AND rcs.referral_code IN (SELECT id_referral_code FROM referral_code as rc WHERE rc.value = :refCode)", nativeQuery = true)
	Collection<Scenario> findByLanguageAndReferral(@Param("lang") String lang, @Param("refCode") String refCode);

	@Query(value = "SELECT s.* FROM scenario as s JOIN referral_code_scenario as rcs on s.id_scenario = rcs.scenario JOIN narrative as n ON n.id_narrative = s.id_narrative WHERE s.lang = :lang AND rcs.released = 1 AND s.deleted = 0 AND rcs.referral_code IN (SELECT id_referral_code FROM referral_code as rc JOIN user as u ON rc.id_referral_code = u.referral_code WHERE cookie = :id_user AND rc.value = :refCode) AND n.id_narrative = :idNarrative", nativeQuery = true)
	Collection<Scenario> findByLanguageUserAndNarrative(@Param("lang") String lang, @Param("id_user") String user_id,  @Param("refCode") String refCode, @Param("idNarrative") int idNarrative);
}
