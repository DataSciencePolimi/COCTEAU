...
public class ScenarioManagementController {
	...
	public String createScenario(Model model, @ModelAttribute("scenarioDescription") String description, @RequestParam("narrative") int idNarrative,
			@RequestParam("scenarioName") String title, @RequestParam("img") MultipartFile img, @RequestParam(name = "coupleScenario", required = false, defaultValue = "0") int scenarioGroup, @RequestParam("imgCredits") String imgCredits,
	...
	@ResponseBody
	@RequestMapping(value = "/getScenarioVisionCount", method = RequestMethod.GET)
	public int getScenarioVisionCount(Model model, @RequestParam("idScenario") int idScenario) {
		
		Scenario scenario = scenarioRepository.findById(idScenario).get();
		
		return scenario.getVisions().size();
	}
